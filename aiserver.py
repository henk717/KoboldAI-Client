...
def general_startup(override_args=None):
    ...
    allowed_ips = sorted(allowed_ips, key=lambda ip: int(''.join([i.zfill(3) for i in ip.split('.')]))) # sort the allowed_ips list in ascending order
    print(f"Allowed IPs: {allowed_ips}") # print out the allowed_ips list
...
def is_allowed_ip():
    ...
    if request.path != '/genre_data.json':
        ...
        print("Allowed?: ",  client_ip in allowed_ips)
